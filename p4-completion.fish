set -l p4_client_commands protect move unlock filelog istat integrate client reshelve revert rename ignores delete aliases shelve reviews list depot flush changelist fix opened prune labelsync edit reconcile workspace login2 print info cstat attribute labels stream protects switch describe login counter lock trust dirs sizes resolve streams key depots clients annotate users undo diff2 tag grep logger tickets branch diff user reopen passwd set resolved clean changelists where submit merge interchanges keys logout populate workspaces unshelve fixes repos fstat rec jobs label repo change branches update sync files counters status integrated job have changes group groups copy review add
...
set -l p4c_help usage commands views dvcs replication filetypes legal jobview administration environment charset configurables simple streamintro revisions command networkaddress
