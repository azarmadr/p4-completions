<<<<<<< HEAD
set -l p4_client_commands counter review reshelve info add reviews edit repo have integrate change submit istat prune undo status labels protects copy streams merge list describe label group unshelve trust sizes revert reopen interchanges reconcile changes integrated clients passwd rec ignores login branches aliases opened groups populate diff fstat sync shelve login2 protect resolve resolved user delete key client fix branch unlock lock workspaces diff2 stream filelog changelist changelists tickets rename where attribute files depot users cstat logout switch print flush annotate repos move workspace counters set clean logger job jobs tag grep update keys depots labelsync dirs fixes
...
set -l p4c_help simple networkaddress command jobview configurables usage environment administration streamintro legal commands replication revisions filetypes charset views dvcs
...
=======
set -l p4_client_commands unshelve logger aliases istat review groups have flush clean tag interchanges unlock undo changelist dirs lock counter where branch key update labels labelsync job repo annotate copy sync submit workspace info protect describe prune fstat integrate grep change branches set merge reviews depot keys passwd client users filelog list diff status attribute login switch label jobs trust print depots sizes rec add move opened workspaces resolve streams populate reshelve counters reopen resolved reconcile logout rename edit ignores fix diff2 protects shelve files changes stream repos changelists user delete login2 revert group clients integrated cstat tickets fixes
...
complete -f -c p4 -n "__fish_seen_subcommand_from help; or not __fish_seen_subcommand_from $p4_client_commands" -a tickets -d "Display list of session tickets for this user"
complete -f -c p4 -n "__fish_seen_subcommand_from help; or not __fish_seen_subcommand_from $p4_client_commands" -a fixes -d "List what changelists fix what job"
set -l p4c_help replication configurables dvcs environment views networkaddress usage command simple jobview revisions filetypes streamintro commands administration charset legal
