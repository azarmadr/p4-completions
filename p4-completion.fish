set -l p4_client_commands workspaces key workspace streams shelve diff2 dirs flush add sync reshelve aliases change attribute info jobs integrated reviews fix passwd branches have stream unshelve group protects depots move lock clients fixes print diff resolve annotate counters repos cstat populate job trust describe set login2 labelsync edit users submit delete interchanges grep where switch reopen changelist opened files prune changes labels clean unlock undo status repo protect login merge list integrate branch logger keys changelists fstat copy tag update revert review label rename logout depot groups tickets user sizes client resolved reconcile rec counter filelog ignores istat
...
set -l p4c_help administration dvcs streamintro replication views simple legal usage charset commands command revisions environment networkaddress configurables jobview filetypes
