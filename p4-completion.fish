set -l p4_client_commands annotate rec fixes copy passwd delete diff filelog protect sync key tag label attribute status reshelve update logout undo flush workspaces logger info group istat login change user labelsync changelist jobs changes list review add resolved shelve print reopen cstat rename keys clients fstat ignores tickets counter groups sizes move diff2 reviews login2 integrate resolve opened integrated labels lock set repo fix grep submit have unlock repos dirs workspace revert changelists merge client where streams edit describe job reconcile stream clean prune users trust populate protects depot unshelve aliases depots interchanges switch files counters branches branch

complete -f -c p4 -n "__fish_seen_subcommand_from help" -a help -d "Print the requested help message"
complete -f -c p4 -n "__fish_seen_subcommand_from $p4_client_commands" -l explain -d "display additional information about the flags"
...
set -l p4c_help environment administration networkaddress replication charset filetypes views commands streamintro revisions simple jobview command dvcs configurables legal usage
